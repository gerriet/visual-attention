 ...
 */
inline cv::Mat visualize_feature_map(
    const core::FeatureMap& feature,
 ...
 */
inline cv::Mat visualize_saliency_map(
    const core::SaliencyMap& saliency,
    const cv::Mat& original = cv::Mat(),
    const std::string& window_name = "Saliency Map",
    bool mark_peaks = false,
    bool wait_key = false)
{
 ...
 */
inline cv::Mat visualize_side_by_side(
    const std::vector<cv::Mat>& images,
    const std::vector<std::string>& labels = {},
    const std::string& window_name = "Comparison",
    bool wait_key = false)
{
    if (images.empty()) {
        return cv::Mat();
