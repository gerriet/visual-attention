...
experimental findings, its central claim under a protocol with predictions
...
\todo{Not yet replicated; found late (it is in thesis \S9.2 and is the main
result of \citealp{backer2003}, and the dossier, built from chapters 5--6, missed
it). Simulated two-dimensional master maps: $5\times5$-pixel objects, static or
moving by at most 2~px per frame, at least 14~px apart, uniform noise of half the
object amplitude; 1, 3 or 5 static and 0--5 dynamic objects; 50 runs of 40
frames per condition. A conventional model (blur, inhibit, maximum; simulated
recognition in 3 frames; an $8\times8$ inhibition mark decaying by 0.8 per frame;
identity bound to the location of recognition) against the two-stage model in its
simplest variant (one 2D field of local inhibition, object files on its activity
clusters, exploration behaviour, recognition in 4 frames). Measures: mean number
of objects whose identity \emph{and} position (within 20~px) the world model
holds, and their mean position error. Reported: the conventional model leads
without dynamic objects; with any, the two-stage model leads and scales better;
position error $<0.5$~px against 0.5--5~px. This measures what the system
\emph{knows}, where the study below measures where attention \emph{goes}; it is
also the one experiment defined on the chain field $\to$ clusters $\to$ object
files (see Limitations). To be replicated as finding 22, with intervals.}
...
\begin{itemize}
  ...
  \item \textbf{Object files are not formed from the neural field in Study~2.}
        In the thesis the first selection stage is the field, and object files
        are created for its activity clusters; the reimplementation's second
        stage segments the fused saliency map instead (a documented
        approximation). Study~2 therefore tests object-based inhibition over
        object files, not the thesis's complete chain. \todo{the replication of
        \S9.2 closes this.}
