...
needs in a dynamic scene---was argued from a handful of demonstrations. Twenty
years later I reimplemented the model and tested it again: 24 of the thesis's own
...
\todo{Why re-test now. (i) The claim was never tested with statistics: the thesis
argues from demonstrations. (ii) The idea---a small symbolic world model that
...
(occlusion regime, Table~\ref{tab:h1}: 2.8 labels per attended object with the
thesis's correspondence, 1.5 with persistent identity---identity is not reliably
kept by the thesis's correspondence alone).
...
thesis's field experiments under both.
...
\arm{object-ior} & object files & the thesis's nearest-centroid correspondence \\
\arm{object-ior+aids} & object files & + motion prediction, appearance \\
\arm{object-ior+id} & object files & + persistent identity \\
\bottomrule
\end{tabularx}
\end{table}
...
inhibition on latency and on staleness in all three regimes, with the thesis's
own nearest-centroid correspondence already. Against the strengthened baseline,
...
from 5.5 to 1.7 at high speed and adds a further 0.4 frames; the thesis's
correspondence carries most of the advantage.
...
``collapses''; the multi-scale explanation of stereo robustness; identity through
occlusion with nearest-centroid correspondence (2.8 labels per object). Not a
