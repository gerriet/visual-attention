...
int main(int argc, char** argv)
{
  ...
  for (int y = 0; y < dummy_feature.rows; ++y)
  {
    for (int x = 0; x < dummy_feature.cols; ++x)
    {
      ...
      float max_dist =
          std::sqrt(dummy_feature.cols * dummy_feature.cols / 4.0f + dummy_feature.rows * dummy_feature.rows / 4.0f);
  ...
  cv::Mat feature_vis =
      attention::visualization::visualize_feature_map(test_feature, display ? "Feature Map (Gradient)" : "", false);
  ...
  cv::Mat overlay_vis =
      attention::visualization::visualize_saliency_map(saliency, image, display ? "Saliency Overlay" : "", true, false);
